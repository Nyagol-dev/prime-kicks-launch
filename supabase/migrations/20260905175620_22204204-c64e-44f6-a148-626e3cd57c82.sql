...
-- first signed-up user becomes admin (the shop owner)
...
CREATE POLICY "lookbook public read" ON public.lookbook_photos FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY "lookbook admin write" ON public.lookbook_photos FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin')) WITH CHECK (public.has_role(auth.uid(),'admin'));
...
CREATE POLICY "settings admin all" ON public.settings FOR ALL TO authenticated
  USING (public.has_role(auth.uid(),'admin')) WITH CHECK (public.has_role(auth.uid(),'admin'));
INSERT INTO public.settings (key, value) VALUES ('callmebot_phone',''), ('callmebot_apikey','');

-- CHECKOUT
CREATE OR REPLACE FUNCTION public.place_order(
  p_name text, p_phone text, p_delivery_method text, p_address text, p_notes text, p_items jsonb
...
BEGIN
  ...
  IF length(trim(p_phone)) < 9 THEN RAISE EXCEPTION 'Please enter a valid phone number'; END IF;
  ...
  VALUES (trim(p_name), trim(p_phone), p_delivery_method, nullif(trim(coalesce(p_address,'')),''), nullif(trim(coalesce(p_notes,'')),''), 0)
  RETURNING id, public.orders.order_code INTO v_order_id, v_code;

  FOR it IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_qty := greatest(1, least(20, (it->>'quantity')::int));
    v_size := it->>'size';
    SELECT * INTO v_product FROM public.products WHERE id = (it->>'product_id')::uuid AND is_active;
    ...
    VALUES (v_order_id, v_product.id, v_product.name, v_size, v_qty, v_product.price);
...
GRANT EXECUTE ON FUNCTION public.place_order(text,text,text,text,text,jsonb) TO anon, authenticated, service_role;

ALTER PUBLICATION supabase_realtime ADD TABLE public.orders;

-- SEED
WITH p AS (
  INSERT INTO public.products (name, slug, description, price, category, images, is_featured) VALUES
